...
import { Play, RotateCcw, Clock, Sun, Moon, Download } from 'lucide-react';
  ...
  return (
    <div className="flex h-screen bg-white dark:bg-black text-slate-900 dark:text-slate-200 font-sans overflow-hidden transition-colors duration-200">
      ...
      <div className="flex-1 flex flex-col min-w-0">
        ...
        {/* IDE Header */}
        <header className="h-14 bg-white dark:bg-slate-900 border-b border-slate-200 dark:border-slate-700 flex items-center justify-between px-6 shadow-sm z-10">
          <div className="flex items-center space-x-4">
             <h2 className="font-semibold text-slate-900 dark:text-white truncate max-w-[300px]">
               {currentProblem?.title} <span className="text-xs font-normal text-slate-500 ml-2 border border-slate-700 px-1.5 py-0.5 rounded">{selectedExamLevel}</span>
             </h2>
          ...
          <div className="flex items-center space-x-3">
             ...
             <button 
                ...
                className="flex items-center px-3 py-1.5 text-xs font-medium text-slate-500 hover:text-slate-900 dark:text-slate-400 dark:hover:text-white bg-slate-100 hover:bg-slate-200 dark:bg-slate-800 dark:hover:bg-slate-700 rounded transition-colors"
             ...
             <button 
                ...
                className={`flex items-center px-4 py-1.5 text-xs font-bold text-white rounded shadow-sm transition-all
        ...
        <div className="flex-1 flex overflow-hidden">
          ...
          <div className="w-5/12 border-r border-slate-200 dark:border-slate-700 min-w-[300px] flex flex-col bg-slate-50 dark:bg-slate-900">
             {currentProblem && <ProblemDetail problem={currentProblem} />}
          </div>

          {/* Right Panel: Editor & Console */}
          <div className="flex-1 flex flex-col min-w-[400px] bg-slate-50 dark:bg-slate-900">
            {/* Editor Area */}
            ...
            <div className="h-2/5 min-h-[200px] max-h-[50%]">
              <Console 
                status={status} 
                output={output} 
