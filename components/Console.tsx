...
import { Terminal, CheckCircle, XCircle, Loader2, ListChecks, FileText, Copy, Trash2, ChevronDown, ChevronRight, Check, Play, Keyboard } from 'lucide-react';
...
export const Console: React.FC<ConsoleProps> = ({ status, output, testResults, onRunCustom }) => {
  const [activeTab, setActiveTab] = useState<'output' | 'tests' | 'custom'>('tests');
  ...
  React.useEffect(() => {
    if (status === ExecutionStatus.COMPLETED) {
      ...
      } else if (activeTab !== 'custom') { // Don't switch away from custom if that's what we ran
        setActiveTab('output');
      }
      setCleared(false);
  ...
  const handleRunCustomClick = () => {
    if (onRunCustom) {
      onRunCustom(customInput);
    }
  };
          ...
          <div className="p-4 h-full flex flex-col">
            <div className="mb-4">
              ...
              <textarea
                ...
                className="w-full h-32 bg-white dark:bg-slate-800 border border-slate-200 dark:border-slate-700 rounded-md p-3 text-sm font-mono text-slate-800 dark:text-slate-200 focus:ring-2 focus:ring-blue-500 outline-none resize-none"
            ...
            <div className="flex justify-end">
              <button
                ...
                className={`flex items-center px-4 py-2 text-xs font-bold text-white rounded shadow-sm transition-all
                  ${status === ExecutionStatus.RUNNING
            ...
            {output && activeTab === 'custom' && (
              <div className="mt-4 flex-1 flex flex-col min-h-0">
                <label className="block text-xs font-semibold text-slate-500 dark:text-slate-400 mb-2">Output</label>
                <div className="flex-1 bg-slate-100 dark:bg-slate-800/50 border border-slate-200 dark:border-slate-700 rounded-md p-3 overflow-auto">
                  <pre className="text-slate-800 dark:text-slate-300 whitespace-pre-wrap break-words font-mono text-sm leading-relaxed">
                    {output}
                  </pre>
                </div>
